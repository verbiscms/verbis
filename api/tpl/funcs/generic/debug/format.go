...
func (f *formatState) formatPtr(v reflect.Value) {
	...
	f.wrt.Write(openAngleBytes)
	f.wrt.Write([]byte(strings.Repeat("*", indirects)))
	f.wrt.Write(closeAngleBytes)
	...
	case cycleFound:
		f.wrt.Write(circularShortBytes)
...
func (f *formatState) Format(v reflect.Value) {
	...
	case reflect.Invalid:
		f.wrt.Write(invalidAngleBytes)
	case reflect.Bool:
		printBool(f.wrt, v.Bool())

	case reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64, reflect.Int:
		printInt(f.wrt, v.Int(), 10)

	case reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64, reflect.Uint:
		printUint(f.wrt, v.Uint(), 10)

	case reflect.Float32:
		printFloat(f.wrt, v.Float(), 32)

	case reflect.Float64:
		printFloat(f.wrt, v.Float(), 64)

	case reflect.Complex64:
		printComplex(f.wrt, v.Complex(), 32)

	case reflect.Complex128:
		printComplex(f.wrt, v.Complex(), 64)
	...
	case reflect.Map:
		...
		for i, key := range keys {
			...
			f.wrt.Write(colonBytes)
			f.Format(f.unpackValue(v.MapIndex(key)))
		}

		f.depth--
		f.wrt.Write(closeMapBytes)
