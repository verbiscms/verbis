...
func (m *Model) FilterRows(q *builder.Sqlbuilder, filters map[string][]params.Filter, table string) error {
	...
	if len(filters) != 0 {
		...
		for column, v := range filters {
			...
			var exists bool
			err := m.DB.QueryRow("SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ? AND COLUMN_NAME = ?", table, column).Scan(&exists)
			if !exists || err != nil {
				return &errors.Error{
					Code:      errors.INVALID,
			...
			for _, filter := range v {
				...
				q.WhereRaw(fmt.Sprintf("(%s%s %s '%s')", fTable, column, operator, value))
			}
			counter++
		}
	}
