...
func FilterRows(driver Driver, filters map[string][]params.Filter, table string) error {
	...
	if len(filters) != 0 {
		...
		for column, v := range filters {
			...
			for _, filter := range v {
				...
				driver.Builder().WhereRaw(fmt.Sprintf("(%s%s %s '%s')", fTable, column, operator, value))
			}
			counter++
		}
	}
