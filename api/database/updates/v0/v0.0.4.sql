...
ALTER TABLE media
    ADD COLUMN (`file_id` INT NOT NULL);

##############################################
# File ID Inserts
##############################################

UPDATE `media` INNER JOIN `files` ON media.uuid = files.uuid
SET media.file_id = files.id
WHERE media.uuid = files.uuid;
UPDATE `media_sizes` INNER JOIN `files` ON media_sizes.uuid = files.uuid
SET media_sizes.file_id = files.id
WHERE media_sizes.uuid = files.uuid;
